...
\item
  ...
  \item
    ...
    $\Delta_1 \vdash u_1: B_1 = \phi(l \conc (1)), \ldots, \Delta_n \vdash u_n: B_n = \phi(l \conc (n))$
  \end{enumerate}
...
\begin{definition}[Values and total term]
  ...
  \item
    ...
    if and only if $t \safeReducesAst u$ for some $u\in \Nat$.
  \item
...
$\vec{v}$ is \emph{trace-compatible}  if and only if it satisfies the following condition:
...
\item
  ...
  By definition of $u$ closed total, we have $t \safeReduces u \safeReducesAst n$ for some $n\in\Num$. 
  Hence $t:\N$ is total.
\item
...
\begin{proof}
  ...
  $t_i = t \restr l_i$ is a subterm of $t$
  and $(\vec{v_1},\ldots,\vec{v_i})$ is a trace-compatible assignment for the node $l_i$. 
  We have to find some $(l_i,\vec{d_i},\vec{a_i})$ such that:
  \begin{itemize}
  \item[(i)]
%15:38 19/04/2024
   $t_{i} = t \restr l_{i}$ is a subterm of $t$,
    $\Label(\Pi,l_i) = \vec{x_{i}}:\vec{D_{i}}\vdash t_{i} : \vec{A_{i}}\rightarrow\N$, 
  ...
  \item[(iv)]
    ...
    $\vec{d_1},\vec{a_1}$, \ldots, $\vec{d_i},\vec{a_i}$) is trace compatible.
    That is, if $i-1$ is a progress point, namely if $t_{i-1}$ is of the form $\cond(f,g)$ and $e_i=2$ 
    (hence $t_i$ is $g$),
    then $\vec{a_i} = \Succ(m')\vec{a'}$, $\vec{d_i} = \vec{d_i}m'$.
...
\item
  ...
  $t[d_{i,\phi(1)}/x_1,\ldots,d_{i,\phi(n)}/x_n]\vec{a_i} = t[d_{i,1}/x'_1,\ldots,d_{i,m}/x'_m]\vec{a_i}$ is not total,
...
\item
  ...
  By the induction hypothesis, $\cond(f[\vec{d_i}],g[\vec{d_i},x])m\vec{a_*}:\N$ is not total,
