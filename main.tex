...
% in all other cases: $\Succ $, $\lambda$, $\etaRule$, $\cond$, and $\ap$ if different from the previous case.
\end{enumerate}
\end{enumerate}
\end{definition}
...
\begin{proof}
  ...
  where $r$ is the root node of $\Pi$. Then we have $t_0 = t$, $(b)$ and $(c)$. 
  ...
  We obtain $(a)$, $(b)$, and $(c)$ for $i+1$, as expected.
  We also have $(d)$ since the connection, determined by $f$, from
