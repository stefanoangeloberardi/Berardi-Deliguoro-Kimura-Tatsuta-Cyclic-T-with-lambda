...
variables,  $\lambda$ (the binder for defining maps), 
...
In this paper will consider the subsets $\CTlambda \subset \GTC, \Reg \subset \WTyped$,
with $\WTyped$ the set of well-typed terms of $\LAMBDA$. 
$\GTC$ is the subset of well-typed circular 
...
regural trees. We will prove that $\CTlambda$ is a decidable subset of $\Reg$.
...
all $n \in \Nat$ are represented in $\LAMBDA$ by expressions we call numerals.
...
\item
$t$ is an immediate subterm of $\Succ(t)$, $f$, $g$ are  immediate subterm of $\cond x.(f,g)$
and $t$, $u$ are  immediate subterm of $t(u)$. The subterm relation is the reflexive and transitive
closure of the immediate subterm relation.
...
The term $t = \cond x.(0,t)$ is in $\LAMBDA$. 
...
\item
The types of $\LAMBDA$ are all types we can define with $\N$, an infinite list 
$\alpha,\beta,\ldots$ of type variables and $\rightarrow$.
...
then $\Gamma \vdash \cond x.(f,g) : \N \rightarrow T$.
\end{enumerate}
...
For instance if $t=\cond x.(t,t)(0)$ we can prove $x:\N \vdash t:A$ for all types $A \in \Type$. 
...
if we never reduce in proper subterms of any $\cond x.(f,g)$. 
...
and $u[\vec{x}]:\Gamma\vdash B$.
  ...
  By the induction hypothesis, $\cond x.(f[\vec{d_i}],g[\vec{d_i},x])m\vec{a'}:\N$ is not total,
...
$r=(\lambda x:T.b)(u)$ or $r=(\cond x.(f,g))(t)$, where $t=0$ or $t=\Succ(u)$. 
We first check that no trace $\tau$ starting from $r$ can progress in the next sub-term of the path.
Indeed, if $\tau$ progresses, then $r=(\cond x.(f,g))(t)$
and $\tau$ to the type of the variable $x$ bound by $\cond x.(f,g)$. 
