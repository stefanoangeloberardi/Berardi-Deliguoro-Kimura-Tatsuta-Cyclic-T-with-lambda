...
$\phi:T \rightarrow L$. If $l \in T$ then we call $\phi(l)$ the label of $l$. 
\item
We define $\setT \restr l =
(T \restr l, \phi_l)$ and $\phi_l(m) = l \conc m$. We call $\setT \restr l$ the labeled subtree of $\setT$
in the node $l$, immediate sub-tree if $l=(i)$. 
\end{enumerate}
\end{definition}

Remark that the labeling of a node $m \in T \restr l$ 
is the same label of the corresponding node $l \conc m$ of $T$.
...
with the root labeled $l$ and $\setT \restr (i) = \setT_i$ for all $1 \le i \le n$.
...
\]
Formally, $\Pi=(T,\phi)$ is defined by $T=$ the same tree of $t$, 
$\phi(l)=(\vdash A)$ if $t \restr l = t$, 
$\phi(l) = (\vdash \N \rightarrow A)$ if $t \restr l = u$
and $\phi(l)=(\vdash\N)$ if $t \restr l = 0$. 
%09:06 24/04/2024
\\
...
We draw the type inference in the picture below. 
We include a back edge from the 
$\bfColor{oldgold}{\dagger}$ on the top to the $\bfColor{oldgold}{\dagger}$ in the bottom,
and we marked $\bfColor{oldgold}{ \spadesuit}$ the only progress point, which is cyclically repeated
in $\tau$.
We abbreviate 
...
Here is an informal definition. Values of type $\N$ are numerals. 
Total terms $t$ of type $\N$ are
the closed terms of type $\N$ evaluating in at least one reduction path to a numeral. 
Total terms $t$ of type $\N$ include values of type $\N$. 
...
$\vec{v}$ is \emph{trace-compatible}  if it satisfies the following condition. 

  For all $j$  index of an argument of $t_i$ assigned to $\Succ^{a}(0)$, 
  all $k$ index of an argument of $t_{i+1}$ assigned to $\Succ^{b}(0)$, 
...
$\rho$ is a value assignment to $\pi$.
\begin{enumerate}
\item
...
\item
  ...
  trace-compatible: all connected $\N$-argument of $e_{i}$ and $e_{i+1}$ are the same,
...
\item
  ...
  trace compatible: all connected $\N$-argument of $e_{i}$ and $e_{i+1}$ are 
