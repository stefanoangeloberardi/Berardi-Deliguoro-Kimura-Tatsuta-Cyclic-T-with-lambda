...
Then $t$ is total of type $T$ if $t \WTyped$, $t:T$ and $t$ is finite for safe reductions.

\item
Let $T$ be any arrow type $A \rightarrow B$.
Then total terms $f$ of type $T$ are are all $f \WTyped$, $f : T$ 
such that for all total $a$ of type $A$ we have $f(a)$ total of type $B$.
\end{enumerate}

A \emph{total assignment} $\vec{v}$ is any assignment of total terms to variables of the same type.

A term is \emph{total by substitution} if all total assignment to its free variable produce a total term.
\end{definition}

By definition, any total term is well-typed, in particular it has exactly one type.

We call the $\Succ$-length of a term $t \in \LAMBDA$ 
the maximum (finite or infinite) of $k$ such that $t \reduces \Succ^k(u)$ for some $u \in \LAMBDA$.
For instance, $0$ has $\Succ$-length $0$, $\Succ(x)$ has $\Succ$-length $1$, 
...
\begin{enumerate}
\item
$t$ has a \emph{finite} $\Succ$-length
\item
If $t \reduces \Succ(u)$, then the $\Succ$-length of $u$ is smaller than the $\Succ$-length of $t$.
\end{enumerate}
\end{lemma}

\begin{proof}
Assume $t$ is finite for safe reductions.\begin{enumerate}
\item
If we always reduce the first safe redex of $t$ we define a reduction sequence with finitely many step
because $t$ is finite for safe reductions. We produce a reduction $t \reduces u$ for some safe normal $u$.
All redexes in $u$ are inside the right-hand side of some maximal $\cond$ in $u$: we have 
$u = v[\cond(f_1,g_1), \ldots, \cond(f_n,g_n)]$ with all redexes inside some $g_1, \ldots, g_n$.
Any reduction sequence on $u$ is applied to $g_1, \ldots, g_n$.

Suppose $t \reduces \Succ^k(v)$. By Church-Rosser, $\Succ^k(v)$ and $u$ are confluent.
Therefore $\Succ^k(v) \reduces \Succ^k(v')$ and  
$u \reduces v[\cond(f_1,g'_1), \ldots, \cond(f_n,g'_n)]$
and $\Succ^k(v') = v[\cond(f_1,g'_1), \ldots, \cond(f_n,g'_n)]$. We deduce that $k$ is less or equal
than the longest prefix of $\Succ$ in $v$. This latter is the $\Succ$-length of $t$.

\item
Assume that $t \reduces \Succ(u)$, in order to prove that 
the $\Succ$-length of $u$ is smaller than the $\Succ$-length of $t$.
By the point above, $u$ has $\Succ$-length some $k \in \N$. Then $u \reduces \Succ^k(v)$,
therefore $t \reduces \Succ^{k+1}(v)$ and the $\Succ$-lenght of $t$ is $\ge k+1$.

\end{enumerate}
\end{proof}

We can assign total terms to  the sub-terms of $t$
...
the numeral $n$ associated to the progressing argument decreases.
...
Total terms are closed by reductions and by application and variables. 



\begin{lemma}\label{lem:total_value-finiteness}
  \begin{enumerate}
  \item\label{lem:total_value-finiteness1}
    Let $t:A$ and $t \reduces u$.
    If $t$ is total, then $u$ is total.
  \item\label{lem:total_value-finiteness2}
    If $f:A \rightarrow B$ and $a:A$ are total terms, then $f(a)$ is total.
  \item\label{lem:total_value-finiteness2bis}
    $x^T:T$ is total.
  \item\label{lem:total_value-finiteness3}
    Let $T$ be any atomic type, and $t[\vec{x}]:\vec{A}\rightarrow T$ be a term
    whose all free variables are $\vec{x}:\vec{B}$.
    be total terms.
    If for all total terms $\vec{u}:\vec{B}$ and $\vec{a}:\vec{A}$ the term 
    $t[\vec{u}]\vec{a}:\N$ is total, then $t[\vec{x}]$ is total by substitution. 
  \end{enumerate}
\end{lemma}
...
\item
  ...
  By definition of $t$ total all reductions from $t$ only include finitely many safe reductions.
  All reduction from $u$ can be extended to a reduction from $t$ and therefore 
  only include finitely many safe reductions.
\item
  We show the \emph{induction case}, namely when $A = (A_1\rightarrow A_2)$.
  Take any arbitrary total term $a:A_1$. Then we have $t(a) \reduces u(a)$ and 
  $t(a):A_2$ is total by the assumption that $t$ is total.
  Hence by the induction hypothesis that $u(a)$ is total. 
...
If we consider the substitution of a variable with itself (a variable is a total term),
...
\begin{proof}
  ...
  there exist total terms $\vec{a}:\vec{A}$ and $\vec{d}:\vec{D}$ such that
...
\item  
  ...
  by the induction hypothesis $\Succ(u)[\vec{d_i}]:\N$ is not total, it has an infinite reduction with
  infinitely many safe reductions.
  Then, by the definition of total, $u[\vec{d_i}] =t_{i+1}[\vec{d_i}] $ is not total, because the
  same infinite reduction applies to $u$.
  We define $e_{i}=1$, 
...
\item
  ...
  the premise of the rule are $\Gamma\vdash f[\vec{x}]: B \rightarrow \vec{A}\rightarrow\N$ 
  and $\Gamma\vdash u[\vec{x}]: B$, where $u$ is \underline{not} a variable.
  By the induction hypothesis, 
  $t_i\vec{d_i}[]\vec{a_i} = f[\vec{d_i}](u[\vec{d_i}])\vec{a_i}:\N$ is not total.
...
\begin{enumerate}
\item
  We first consider the subcase that \emph{$u[\vec{d_i}]:B$ is total}.
  ...
  Using Lemma \ref{lem:total_value-finiteness}.\ref{lem:total_value-finiteness2}
...
\item
  ...
  in $\vec{d_i}$ and therefore are the same, no unnamed arguments are connected.
 \end{enumerate}
...
\item
  ...
  $t_i[\vec{d_i}]\vec{a_i} = (\lambda x.(u[\vec{d_i},x]))\vec{a_i}:\N$ not total,
  where $\vec{a_i} = a,\vec{b}$. There is some infinite reduction $\sigma$ from 
  $t_i[\vec{d_i}]\vec{a_i}$. 
...
\begin{enumerate}
\item
 Suppose all reductions in $\sigma$ are inside $\lambda x.(u[\vec{d_i},x])$ or $\vec{a_i}$.
 Then there are finitely many safe reductions in $\vec{a_i}$ because $\vec{a_i}$ is total.
 Thus, there are infinitely many safe reductions on $\lambda x.(u[\vec{d_i},x])$.
 We define $\vec{d_{i+1}} = x,\vec{d_i}$ and $\vec{a_{i+1}} = \vec{a_i}$. 
\item
...
\item
  ...
  By the induction hypothesis,
  $t_i[\vec{d_i}]m \vec{b} = \cond(f[\vec{d_i}],g[\vec{d_i}])m\vec{b}:\N$ is not total,
  where $\vec{a_i} = m,\vec{b}$. There is some reduction sequence $\sigma$ with infinitely many 
  safe reductions from $t_i[\vec{d_i}]m \vec{b}$. If the reduction sequence never contracts
  the leftmost $\cond$-redex, then infinitely many reductions take place on $f[\vec{d_i}]$, 
  $m$, $\vec{b}$, but not on $g[\vec{d_i}]$ (they would not be safe).
  $m$, $\vec{b}$ are total, only finitely many reduction on them are possible, 
  therefore we have a contradiction.
  If the reduction sequence contracts the leftmost $\cond$-redex, then 
  $t_i[\vec{d_i}]m \vec{b} \reduces \cond(u,v)m"\vec{b'}$, with $m"=0, \Succ(m')$,
  then $\cond(u,v)m" \reduces v m' \vec{b'}$ or $\cond(u,v)m" \reduces \vec{b'}$.
...
%12:53 05/06/2024
 ...
 \item
  Suppose $m" = \Succ(m')$. We choose $l_{i+1} = l_i \conc (2)$ and $\vec{d_{i+1}} = \vec{d_i}$,
  $\vec{a_{i+1}} = m',\vec{a_i}$.
  We have trace-compatibility because: 
  each argument of $t_i[\vec{x}]$ is connected to some equal argument of 
    ...
    as indeed it is the case, because $m \reduces m" = \Succ(m')$.
...
From this theorem we derive the strong normalization result for safe reductions.
...
\begin{corollary}\label{cor:WN_typeN}
  ...
  \item
   \item
   Assume  $\Gamma\vdash t:A$. Then $t \in \GTC$ implies that $t$ safe-normalizes
  \item
