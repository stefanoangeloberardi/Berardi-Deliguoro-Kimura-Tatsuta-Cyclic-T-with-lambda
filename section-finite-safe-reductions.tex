...
Assume $t \in \WTyped$. We say that $t$ is \emph{finite for $n$-safe reductions} if all infinite
...
This is a contradiction when $h$ is larger than $k$, which is the number of symbols in $u$.
...
\begin{corollary}\label{cor:SN_GTC}
  ...
  \item
    $t$ strongly $n$-safe-normalizes.
  \item
    All infinite reduction sequences from $t$ have some limit.
    {\rm{\color{red}\Large Needs to be checked}}
  \end{enumerate}

\end{corollary}

{\color{red} Note that $\CTlambda$ is not closed under infinite reductions. Consider a term $f = \lambda x^\N.\cond(x,f(\Succ x))$ that has a type $\N,\N\rightarrow\N$. It is regular, and also is in $\GTC$. Hence $f\in\CTlambda$.
  Then, by using $fn \reduces \cond(n,f(\Succ n))$, there is a infinite reduction sequence
  \[
  f0\,\reduces\,\cond(0,f1) \,\reduces\,\cond(0,\cond(1,f2)) \,\reduces\cdots. 
  \]
  Hence its limit would be $\cond(0,\cond(1,\cond(2,\cdots)))$ that is not regular,
  since it has infinitely many different subterms $0,1,2,\ldots$. 
  Therefore it is not in $\CTlambda$ even though each term finitely reachable from $f0$ is in $\CTlambda$. 
}

There are closed normal terms of $\N$, even regular terms, which are not numerals.
