...
only finitely many \quotationMarks{$n$-safe} reduction steps.    
In particular,
...
no matter how we reduce within the $n$-safe level of a term, eventually we obtain some $n$-safe normal form.
\\
...
We have to prove that all terms in $\GTC$ are finite for $n$-safe reductions, for all $n \in \N$.
...
Then $t$ is total of type $T$ if and only if $t$ is finite for $n$-safe reductions, for all $n \in \N$.
...
However, we can prove that if $t$ is a term finite for $n$-safe reductions, 
then any $\prec$-decreasing sequence from $t$ terminates.


\begin{lemma}[The $\prec$-order]
\label{lemma-prec-order}
Assume $n \in \N$, $t \in \WTyped$ has type $\N$, and $t$ is finite for $n$-safe reductions .
...
\item
%\label{lemma-prec-order-01}
$t$ is finite for $n$-safe-reductions, therefore
$\sigma$ only has finitely many $n$-safe-reductions. 
Thus, from some $k\in\N$ there are no more $n$-safe reductions from
...
from some $t_0:\N$ total, hence by definition finite for $n$-safe reductions.
...
and that in particular there is some  infinite $\prec$-decreasing chain from some term
finite for $n$-safe reductions.
...
Now we will prove that if an infinite path has a trace-compatible assignment, 
then all traces of the path progress only finitely many times and the term is not in $\GTC$
(does not satisfy the global trace condition).
...
The assignment is made of total terms, therefore
 $\prec$ well-founded by lemma \ref{lemma-prec-order}.\ref{lemma-prec-order-02}.
...
%12:53 05/06/2024
 ...
 \item
  ...
  each argument of $t_i[\vec{x}]$ is connected to some equal argument of 
    ...
    the trace progresses and $m' \prec m$, because $m \reduces  m" = \Succ(m')$.
