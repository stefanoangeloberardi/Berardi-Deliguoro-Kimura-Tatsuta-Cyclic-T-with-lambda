...
From $i \not \in X$ we deduce that $\phi_X(t)$ is, respectively: 
$$ 
x, \ \ \  
0, \ \ \  
\Succ(\phi_X(u)),  \ \ \  
...
\cond(\phi_X(f),\phi_X(g))
$$
Again from $i \not \in X$ we deduce that $\phi_X^2(t))$ is, respectively: 
$$ 
x, \ \ \  
0, \ \ \  
\Succ( \phi_X^2(u) ),  \ \ \  
...
\item
Then we assume that $t$ is some $X$-redex and $t'$ is the $X$-form of $t$ (hence $t'$ is no $X$-redex).
...
\item
Assume that all $X$-applications in $t$ are redexes, 
in order to prove that $\phi_{X}(t)[\phi_{X}(u)/x]= \phi_{X}(t[u/x])$.
We write $\sigma=[a/x]$ and we have to prove $\phi_{X}(t)\sigma= \phi_{X}(t\sigma)$.

By Lemma \ref{lemma-sufficient-condition}, applied to $t$, we have to prove that
$\head( \phi_{X}(t)\sigma ) = \head( \phi_{X}(t\sigma) )$ 
and that either $\phi_{X}(t)\sigma= \phi_{X}(t\sigma)$, or any two immediate subterms
in the same position in $\phi_{X}(t)\sigma$
are of the form $\phi_{X}(t')[\phi_{X}(u)/x]$ and $\phi_{X}(t'\sigma)$ for some $t'$.
