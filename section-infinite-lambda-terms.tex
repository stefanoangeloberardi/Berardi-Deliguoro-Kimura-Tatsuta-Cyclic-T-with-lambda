...
We abbreviate \quotationMarks{$1$-safe normal} with just \quotationMarks{safe normal}.
\end{enumerate}
\end{definition}
...
of $z$ because it is irrelevant. Then $u$ is $1$-safe-normal (safe normal for short), because
...
need $1$-safe reductions, those in the the right-hand-side
of \emph{no} $\cond$, and that $1$-safe reductions strongly normalize. 
...
from $v(n)$, and they are all $1$-safe. $v(n)$ $\cond$-reduces to $v(n-1)$, 
