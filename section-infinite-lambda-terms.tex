...
\begin{definition}[Substitution for terms of $\LAMBDA$]
  A {\em substitution} $\theta$ is a list of the form $u_1/x_1,\ldots,u_n/x_n$ with distinct variables $x_1,\ldots,x_n$.
  A substitution is called {\em renaming} if $u_1,\ldots,u_n$ are distinct variables. 
  We define $\theta(y)$ by $u_i$ if $y=x_i$, and by $y$ otherwise.
  The set $\Dom(\theta)$ is $\{x_1,\ldots,x_n\}$. 
  Let $X$ be a set of variables.
  The substitution $\Restrict{\theta}{X}$ is the restriction of $\theta$ to $X\cap\Dom(\theta)$.
  We abbreviate $\Restrict{\theta}{\Dom(\theta)\setminus\{x\}}$ by 
  $\Restrict{\theta}{\overline{x}}$. 
...
by using the $\weak$-rule (Weakening + Exchange  rule):
 \[
  \infer[\weak]{
...
that all leftmost branches in any sub-proof are finite, in particular that no infinite branch made of
$\weak$-rules exists. 
%12:54 03/06/2024
